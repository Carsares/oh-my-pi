...
OMP's `new_session` and `switch_session` commands replace the live session inside a process. Using a
...
Clients address a runtime with opaque `workspaceId`, OMP `sessionId`, and random `instanceId`. A new
session temporarily uses a Host ID until OMP persists its formal ID. Canonical paths remain private to
...
navigation may change the active branch within that same session through OMP-owned behavior.
...
working or be reused after its OMP process exits. Reopening starts a fresh process from persisted state.
...
appends the name through `appendSessionInfo()`. Active-session rename continues to use OMP's native
