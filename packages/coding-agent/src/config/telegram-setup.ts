...
function getTelegramAccount(config: ChatConfigLike | undefined) {
	return Object.entries(config?.accounts ?? {}).find(([, account]) => account?.service === "telegram");
}
...
export function buildTelegramDoctorReport(
	...
	const listenerStatus = (input.workerStatuses ?? []).find(
		status =>
			status.conversationId === conversationId || (!conversationId && status.conversationId?.startsWith("telegram")),
	);
	...
	const checks: TelegramDoctorCheck[] = [
		{
			...
			message: configured ? "Telegram bot token is saved in Picot chat config." : "Telegram is not connected.",
		},
		{
			id: "bot",
			label: "Bot",
		...
		},
		{
			...
			message: dm?.id ? `Private DM is bound to ${dm.name || dm.id}.` : "No private Telegram DM is bound.",
		},
		{
			id: "security",
			label: "Security",
